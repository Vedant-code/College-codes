...
for (int i = 0; i < n; i++)
{
printf("\nEnter the Name , Roll Number , Division , Address of the Student : "); scanf(" %s %d %c %s", e[i].name, &e[i].roll, &e[i].Div, e[i].Address);
}
n++;
}
...
FILE *fp;
fp = fopen("Student.txt", "r");
struct Student StuData[10]; if (fp == NULL)
{
printf("Error"); return;
}
else
...
int i, j; FILE *fp;
fp = fopen("Student.txt", "r");
struct Student StuData[50]; if (fp == NULL)
{
printf("Error"); return;
}
else
...
printf("Enter the name of Student : "); scanf("%s", name);
FILE *f2;
f2 = fopen("Student.txt", "w"); if (f2 == NULL)
{
printf("Error"); return;
}
else
...
struct Student e[50];
int r = 1;
printf("enter the total number of Students : "); scanf("%d", &n);
while (r)
{
